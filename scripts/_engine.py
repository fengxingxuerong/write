...
# ruff: noqa: F821
# ⚠️ 孤儿切片（2026-09-06 遗留）：本文件引用的词表/工具函数（wc、HOOKS、
# NAMES、CONTINUATION_OPENERS 等）均定义在 demo_novel_gen.py 中，但当前
# 没有任何脚本 import / exec 它，是残缺的历史片段。F821（未定义名）因此
# 大量误报——保留文件供对照，勿在 CI 全量 F 检查中卡死。
# 建议：确认无用后删除，或并入 demo_novel_gen.py（两者明显同源）。
